...
**블라인드 사전 평결 (열람 전 필수)**: 트랙 결과를 열람하기 **전에** PLAN의 성공 기준을 심사 체크리스트로 고정하고, 핵심 질문별 자신의 사전 예상을 한 줄씩 기록한다(내부용, 출력 안 함). 결과 열람 후에는 체크리스트를 완화·수정하지 않는다 — 서브 결과에 휩쓸리는 것을 막는 장치다. 사전 예상과 서브 결과가 크게 다른 지점은 우선 검증 대상으로 삼는다.
