...
1. 브리프의 `[PROVIDER SPEC]` 블록을 읽는다: `tools.call`(호출 도구), `tools.reply`(후속 도구, 선택), `arg_map`(모델/effort 인자 매핑), `capabilities`, `MODEL`, `EFFORT`, `split` 여부.
...
3. 호출 구성:
   ...
   - `capabilities.per_call_model/effort`가 true이고 `arg_map`에 경로가 정의된 인자만 전달한다. `inherit`/`default` 값은 생략한다. 정의가 없거나 capability가 false면 prompt만으로 호출하고 실제값을 inherited/default로 기록한다.
...
6. 브리프에 성공 기준(C{n})이 있으면 자가 채점하고, 미충족 기준은 질문을 좁혀 1회 추가 호출 후 채점을 갱신한다(내부 반복 최대 1회). 자가 채점은 사전 필터일 뿐이다.
7. 표준 형식으로 반환.
...
## 출처
## 프록시 검수 노트
(사용 도구/인자, requested→actual model/effort, 호출 횟수·재시도, 검수 발견 사항. 없으면 "특이사항 없음")
```
