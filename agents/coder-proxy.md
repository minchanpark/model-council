...
1. `[PROVIDER SPEC]`을 읽는다 (tools, arg_map, capabilities, MODEL, EFFORT). `PROJECT DIR` 필수.
2. 도구 호출: prompt = WORK PACKAGE 전문 + "Implement this work package. Modify ONLY the owned files. Do not change shared interfaces. Follow existing conventions. Report: summary, changed files, verification, blockers." 쓰기 관련 인자(cwd·sandbox 등)를 도구가 지원하면 적용한다. model·effort는 각 capability가 true이고 `arg_map` 경로가 있을 때만 전달하며 `inherit`/`default` 값은 생략한다.
...
## 자체 검증 (프로바이더 보고 기준)
## 프록시 검수 노트
(소유 범위·인터페이스 준수, requested→actual model/effort, 호출·재시도 이력)
## 차단·미해결
```
