...
1. 브리프 헤더에서 `CODEX EFFORT`, `CODEX MODEL`, `PROJECT DIR`를 읽는다.
2. `codex` 도구 호출:
   - 인자: `cwd: <PROJECT DIR>`, `sandbox: "workspace-write"`, `approval-policy: "never"`, `config: {"model_reasoning_effort": "<CODEX EFFORT>"}`, `model`(명시 시).
